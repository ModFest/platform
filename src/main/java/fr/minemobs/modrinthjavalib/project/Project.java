...
public class Project {
	...
	public Version[] getVersions() throws IOException {
		try(Response resp = modrinth.getClient().newCall(new Request.Builder().url(modrinth.getBaseURL() + String.format("project/%s/version", this.id)).build())
				.execute()) {
			if(resp.code() != 200) throw new IOException("Failed to delete project. Response code: " + resp.code() + " with message: " + resp.body().string());
	...
	public Version getVersion(String versionID) throws IOException {
		try (Response resp = modrinth.getClient().newCall(new Request.Builder().url(modrinth.getBaseURL() + String.format("version/%s", versionID)).build()).execute()) {
			if(resp.code() != 200) throw new IOException("Failed to delete project. Response code: " + resp.code() + " with message: " + resp.body().string());
			return modrinth.getGson().fromJson(resp.body().string(), Version.class);
		}
	}
	...
	public Dependencies getDependencies() throws IOException {
		try(Response resp = modrinth.getClient().newCall(new Request.Builder().url(modrinth.getBaseURL() + String.format("project/%s/dependencies", this.id)).build()).execute()) {
			if(resp.code() != 200) throw new IOException("Failed to delete project. Response code: " + resp.code() + " with message: " + resp.body().string());
	...
	public void deleteProject() throws IOException {
		try(Response resp =  modrinth.getClient().newCall(new Request.Builder().url(modrinth.getBaseURL() + "project/" + id)
				.delete().addHeader("Authorization", modrinth.getApiKey()).build()).execute()) {
			if(resp.code() != 204) throw new IOException("Failed to delete project. Response code: " + resp.code() + " with message: " + resp.body().string());
		}
	}
