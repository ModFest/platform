...
export async function getAllCredits(platform: Platform, submissions: SubmissionData[]) {
	const submissionSorted = submissions.toSorted((a,b) => a.name.localeCompare(b.name, "en-GB"))
	return {
...
export async function getCredits(platform: Platform, submission: SubmissionData) {
	const authors = await Promise.all(submission.authors.map(a => platform.getUser(a)));
	return {
